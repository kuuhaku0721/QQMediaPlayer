...
using namespace std;                            //TODO;关于libvlc库的封装使用，要总结和学习的还有很多啊..
...
void vlc_callback(const struct libvlc_event_t* p_event, void* p_data)
{
    ...
    if (pThis)
    {
        switch (p_event->type)
        {
        case libvlc_MediaPlayerPositionChanged:  //播放位置（进度）改变
        {
            ...
            int curH = curSecs / 3600;  //时
            int curMinute = (curSecs - curH * 3600) / 60;   //分
            int curSec = curSecs - curH * 3600 - curMinute * 60; //秒

            char buf1[256] = { 0 };
            sprintf(buf1, "%02d:%02d:%02d", curH, curMinute, curSec);   //已播放的
            ...
            char buf2[256] = { 0 };
            sprintf(buf2, "%02d:%02d:%02d", _hour, _minute, _sec);      //总的

            QString str2(buf2);

            QString text = str1 + "/" + str2;       //已播放的 / 总的

            pThis->setTimeText(text);
        }
        break;

        case libvlc_MediaPlayerAudioVolume: //音量改变
        {
        ...
        case libvlc_MediaPlayerMediaChanged:    //媒体文件改变
        {
...
int CVlcKits::initVLC()
{
    ...
    if (m_pInstance)
    {
        ...
        if (m_pMediaPlayer)
        {
            ...
            //将vlc事件管理器绑定到回调函数中，当触发事件时能自动触发回调函数，并在回调函数中分类执行相应功能
...
{
    emit sig_TimeSliderPos(value);          //这些信号都会在MainWindow中绑定响应槽函数
}
