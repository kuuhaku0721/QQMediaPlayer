...
#define TIME_SLIDER_HEIGHT   10			//进度条的高
...
bool CPlayCtrlBar::eventFilter(QObject* watched, QEvent* event)
{
	if (m_pSpeedButton)
	{
		if (event->type() == QEvent::Show && watched == m_pSpeedButton->menu())
		{
			...
			m_pSpeedButton->menu()->move(menuXPos, menuYPos);   //计算按钮的为止，然后移动到合适的地方显示
			return true;
		}
	}
...
void CPlayCtrlBar::setTimeSliderRange(qint64 value)  //时长范围
{
	m_pTimeSlider->setRange(0, value);
}

void CPlayCtrlBar::setSliderValue(qint64 value)  //进度条范围
{
	m_pTimeSlider->setValue(value);
}

void CPlayCtrlBar::showSlider(bool flag) //显示进度条(肯定会有计时器决定它的显示和隐藏)
{
	m_pTimeSlider->show();
}
...
void CPlayCtrlBar::InitData()
{
	...
	//因为设置速率的函数功能很简单，就不需要单独写函数了，直接用lambda表达式完成
	...
	pMenu->setWindowOpacity(0.7);  //设置菜单栏的不透明度
	...
	showTimeLabel(false); //默认不显示时间标签，开始播放后显示,并随播放进度改变数值
