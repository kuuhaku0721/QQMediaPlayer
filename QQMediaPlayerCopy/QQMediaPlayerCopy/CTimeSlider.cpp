...
CTimeSlider::CTimeSlider(QWidget* p)
	:QSlider(p)
{
	setAttribute(Qt::WA_StyledBackground); //样式表设置不受父窗口影响
...
{
	if (event->type() == QEvent::MouseButtonPress && watched == this)  //这个应该是看的最简单的一个事件过滤了
	{//		事件类型				鼠标按下				目标对象		自己
...
void CTimeSlider::slider_mouseLButtonPress(QObject* slider, QEvent* event)
{
	do
	{
		...
		if (Qt::LeftButton != mouseEvent->button())  //限制只能鼠标左键点击
		{
			break;
		}
