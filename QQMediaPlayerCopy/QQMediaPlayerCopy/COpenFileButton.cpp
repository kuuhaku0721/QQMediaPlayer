...
#define LEFT_BUTTON_WIDTH 145   //左侧按钮是'open file'按钮 点击就是打开文件
#define RIGHT_BUTTON_WIDTH 60	//右侧按钮是个选项按钮 点击后显示的是菜单栏 能够选择打开文件、文件夹、网络流等
#define BUTTON_HEIGHT 40		//高固定为40

COpenFileButton::COpenFileButton(QWidget* p) 
	: QWidget(p)
{
	//设置总宽度
	...
	m_pArrowButton = new QPushButton(this); //右侧的一个小箭头按钮，点击打开的是菜单栏
	...
	setWindowFlags(Qt::FramelessWindowHint);	//因为这些按钮依附于的是一个窗口，所以要把窗口设置为无边框模式
